...
public class ParameterExampleJavaTest extends JUnitRouteTest {
    ...
    public void testProcessQueryParameters(){
        testRoute(routeHolder.routes()).run(HttpRequest.GET("/listen?p1=foo&p2=bar"))
    ...
    public void testIgnoreMissingQueryParameters(){
        testRoute(routeHolder.routes()).run(HttpRequest.GET("/listen?p1=foo"))
                .assertStatusCode(StatusCodes.NOT_FOUND);
    }

    @Test
    public void testProcessOptionalQueryParameters(){
        testRoute(routeHolder.routes()).run(HttpRequest.GET("/opt?p1=foo"))
                .assertEntity("p1 -> foo")
                .assertStatusCode(StatusCodes.OK);

        testRoute(routeHolder.routes()).run(HttpRequest.GET("/opt"))
    ...
    public void testProcessPahtParametersAsString(){
        testRoute(routeHolder.routes()).run(HttpRequest.GET("/buy/house"))
    ...
    public void testProcessPahtParametersAsInt(){
        testRoute(routeHolder.routes()).run(HttpRequest.GET("/double/3"))
