...
class LogRequestResponse extends HttpApp with DefaultJsonProtocol {
  ...
  private val lorem = List("lorem", " ", "ipsum", " ", "dolor", " ", "sit", " ", "amet", "\n").map(ByteString.fromString)

  private val infiniteLorems = Source.cycle(() => lorem.iterator)
  ...
  private val logResponseTime: Directive0 = logRequestResult(LoggingMagnet(printResponseTime))

  override protected def routes: Route = logResponseTime {
    concat(
      path("finite") {
        complete("Done!")
      },
      path("infinite") {
        complete(HttpEntity.Chunked.fromData(ContentTypes.`text/plain(UTF-8)`, infiniteLorems))
      }
    )
  }
}
